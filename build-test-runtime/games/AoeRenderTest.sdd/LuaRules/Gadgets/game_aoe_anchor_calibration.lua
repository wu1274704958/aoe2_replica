...
if not gadgetHandler:IsSyncedCode() then
	...
	local function ResetPreview(clearRuntimeOverride)
		local unitDef = UnitDefs[UnitDefNames[CALIBRATION_UNIT_NAME].id]
	...
	local function BuildExport()
		...
		local muzzleCorrection = {
			preview.muzzle[1] - CALIBRATION_BASE_MUZZLE_LOCAL[1],
			preview.muzzle[2] - CALIBRATION_BASE_MUZZLE_LOCAL[2],
			preview.muzzle[3] - CALIBRATION_BASE_MUZZLE_LOCAL[3],
		}
		return string.format([[-- Generated by AOE Anchor Calibration. Review before merging.
-- Source DAT: u_arc_archer collision_size=(0.2,0.2,2.0), weapon_offset=(0,0.5,1.5)
...
return {
	["aoe_archer"] = {
		...
		customParams = {
			...
			CALIBRATION_BASE_MUZZLE_LOCAL[1], CALIBRATION_BASE_MUZZLE_LOCAL[2], CALIBRATION_BASE_MUZZLE_LOCAL[3],
	...
	local function ExportPreview()
		...
		local path = string.format("LuaUI/Config/AOEAnchorCalibration/u_arc_archer_override_%d_%d.lua", Spring.GetGameFrame(), exportSerial)
	...
	function gadget:Initialize()
		...
		Spring.Echo(string.format(
			"[AOE Anchor Calibration] perspective telephoto camera applied=%s fov=%.1f height=%.1f angle=%.1fdeg featureDraw=%.1f featureFade=%.1f",
...
local function SpawnCalibrationScene()
	...
	for index = 1, CALIBRATION_DIRECTION_COUNT do
		...
		local unitID = Spring.CreateUnit(CALIBRATION_UNIT_NAME, x, Spring.GetGroundHeight(x, z), z, "south", 0)
		if unitID == nil then
			error("[AOE Anchor Calibration] failed to create " .. CALIBRATION_UNIT_NAME)
		end
